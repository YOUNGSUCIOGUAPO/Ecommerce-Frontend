...
const CartItems:React.FC = () => {
    ...
    const {cart, removeItemHandler, increaseQuantityHandler, decreaseQuantityHandler, getCartProducts} = useCustomContext();
            ...
            <div className="flex gap-2 text-[14px] mb-20 w-8">
             ...
             {/* header row  */}
                    ...
                    return(
                        ...
                        <div className=" flex items-center gap-[15px] ">
                            <div className="relative"> 
                                {removeItemIndex === cart.id &&(<span onClick={()=>{removeItemHandler(cart.id)}} className="w-6 h-6 bg-[#db0000] absolute right-12 cursor-pointer text-white font-normal text-sm rounded-full flex items-center justify-center">X</span>)}
                         ...
                         <div className="flex justify-center w-[72px] h-[44px] border rounded-[4px] gap-4 items-center px-2">
                            ...
                            <div className="flex flex-col">
                                <Image className="cursor-pointer"  src={DropUp} onClick={()=>increaseQuantityHandler(cart.id)} alt=""/>
                                <Image className={`cursor-pointer ${cart.quantity ===1 && 'opacity-50 pointer-events-none'}`} src={DropDown} onClick={()=>decreaseQuantityHandler(cart.id)}  alt=""/>
                            </div>  
                        </div>
                         </div>
                      
                        <div className="flex justify-self-end items-center">
                            <span className="">{(cart.quantity * cart.price).toFixed(2)}</span>
                        </div>
                ...
                <div className="flex justify-between items-center pt-4">
                    <button onClick={()=>{router.push('/')}} className="border rounded-[4px] p-[16px_48px]">Return To Shop</button>
                </div>
